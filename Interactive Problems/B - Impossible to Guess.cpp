/*
	Problem Link : https://codeforces.com/gym/100488 (B)
	PDF Download : https://codeforces.com/gym/100488/problem/B
	Problem Name : Impossible to Guess
*/
	...
	if(n <= 5){
		...
		for(int i=2 ; i <= total ; i++){
			...
			if(i==2){
				...
				else if(n==6){ // getting wrong here
					int five[2]; int idxf = 0 ; 
					for(int kk = 1 ; kk <= 3 ; kk++) {
						if(ans[4]==a[3][kk]) a[3][kk] = 0 ;
						else {five[idxf] = a[3][kk] ;idxf++; }
					}
					if(!binary_search(a[3],a[3]+4,ans[3])){
						swap(ans[2],ans[3]) ; 
					}
